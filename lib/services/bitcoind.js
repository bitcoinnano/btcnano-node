...
var BitcoinRPC = require('btcnanod-rpc');
...
/**
 * Provides a friendly event driven API to btcnanod in Node.js. Manages starting and
 * stopping btcnanod as a child process for application support, as well as connecting
 * to multiple btcnanod processes for server infrastructure. Results are cached in an
...
Bitcoin.DEFAULT_CONFIG_SETTINGS = {
  ...
  timestampindex: 1,
  spentindex: 1,
  zmqpubrawtx: 'tcp://127.0.0.1:29657',
  zmqpubhashblock: 'tcp://127.0.0.1:29657',
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in btcnanod config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in btcnanod config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in btcnanod config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './btcnano.conf');

  log.info('Using btcnano config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of btcnano-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of btcnano-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of btcnano-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to btcnanod from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from btcnanod. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from btcnanod. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in btcnano.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that btcnanod is undergoing a reindex. ' +
             ...
             'of btcnano-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/btcnano.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/btcnano.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Btcnano Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Btcnano Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/btcnanod.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned btcnano process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean btcnano process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting btcnano process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Btcnano process unexpectedly exited with code:', code);
        log.warn('Restarting btcnano child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Btcnano process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new BitcoinRPC({
        protocol: 'http',
        host: '127.0.0.1',
        port: self.spawn.config.rpcport || 9657,
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn btcnanod.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to btcnanod.'));
    }
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Btcnano configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {String|Address|Array} addressArg - An address string, btcnano address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
 ...
 *   nonce: 2981820714,
 *   solution: '',
 *   bits: '1a13ca10',
...
inputAddress = input.script.toAddress(this.node.network)
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('btcnanod spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('btcnanod process did not exit'));
      }
