...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for btcnanod is necessary for this upgrade with the "reindex=1" btcnano.conf option. \n' +
                 'There are changes necessary in both btcnano.conf and btcnano-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitcoinnano/btcnano-node/blob/bitcoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed btcnano-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './btcnano-node.json');
